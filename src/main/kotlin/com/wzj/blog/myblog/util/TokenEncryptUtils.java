...
public class TokenEncryptUtils {
    ...
    public static String createJWT(String userId, String userInfo, long ttlMillis, long dueto_time) {
        ...
        byte[] apiKeySecretBytes = DatatypeConverter.parseBase64Binary(ENCODED_PASSWORD);
        ...
        if (dueto_time > 0) {
            //设置过期时间
            builder.setExpiration( new Date(dueto_time));
        }
    ...
    public static boolean parseJWT(String jwt) {
        try {
           Claims claim = Jwts.parser()
                   .setSigningKey(DatatypeConverter.parseBase64Binary(ENCODED_PASSWORD))
