...
open class SortsController {
    ...
    @Encrypt
    @ResponseBody
    fun addSorts(@ModelAttribute("data") data: String?, request : HttpServletRequest): BaseData<Any?> {
        logger.info("LanelsController-addSorts",data)
        if (CheckReceivedDataUtil.JsonToClass<SortsEntity>(SortsEntity::class.java,data)==null) return Result1.failure300("格式错误!!!")
        val sortsEntity = CheckReceivedDataUtil.JsonToClass<SortsEntity>(SortsEntity::class.java, data)
        if (sortsEntity==null)  return Result1.failure300("参数不能为空!")

        if (sortsEntity.sortName.isNullOrBlank()) return Result1.failure300("分类名称不能为空!")
        ...
        if (sorts<=0)  return Result1.failure300("添加失败!")
    ...
    @Encrypt
    @ResponseBody
    fun findSortid(@ModelAttribute("data") data: String?, request : HttpServletRequest):BaseData<Any?>{
        logger.info("LanelsController-findSortid",data)
        if (CheckReceivedDataUtil.JsonToClass<SortsEntity>(SortsEntity::class.java,data)==null) return Result1.failure300("格式错误!!!")
        val sortsEntity = CheckReceivedDataUtil.JsonToClass<SortsEntity>(SortsEntity::class.java, data)
        if (sortsEntity==null)  return Result1.failure300("参数不能为空!")

        if (sortsEntity.sortid==0) return Result1.failure300("缺少参数分类ID!")

        val sortid = mainService.lanelsService.queryById(sortsEntity.sortid)
        logger.info("LanelsController-findSortid",sortid)
        return Result1.success200(Gson().toJson(sortid),"查询成功")
    }
    ...
    fun deleteLanels(@ModelAttribute("data") data: String?, request : HttpServletRequest):BaseData<Any?>{
        ...
        //先查 后删除
        if (sortsSize.sortid==0) return Result1.failure300("删除失败,没有找到标签!!")

        val sorts = mainService.sortsService.deleteById(sortsSize.sortid)
        logger.info("LanelsController-deleteLanels",sorts)
        if (sorts <=0) return Result1.failure300("删除失败!")

        return Result1.success200(Gson().toJson(sorts),"删除成功")
    }
    ...
    fun upLanels(@ModelAttribute("data") data: String?, request : HttpServletRequest):BaseData<Any?>{
        ...
        if (sortsStatus<=0) return Result1.failure300("修改失败!")
