...
open class CommentController {
    ...
    @Encrypt
    @ResponseBody
    fun addComment(@ModelAttribute("data") data: String?, request : HttpServletRequest):BaseData<Any?>{
        logger.info("LanelsController-addLanels",data)
        if (CheckReceivedDataUtil.JsonToClass<CommentEntity>(CommentEntity::class.java,data)==null) return Result1.failure300("格式错误!!!")
        val commentEntity = CheckReceivedDataUtil.JsonToClass<CommentEntity>(CommentEntity::class.java, data)
        if (commentEntity==null)  return Result1.failure300("参数不能为空!")
        if (commentEntity.userid==0) return Result1.failure300("缺少用户ID!")
        if (commentEntity.articleid == 0) return Result1.failure300("缺少博文ID!")
        if (commentEntity.commentContent.isNullOrBlank()) return Result1.failure300("评论内容不能为空!")
        ...
        if (size<=0)  return Result1.failure300("评论失败!")
        logger.info("LanelsController-addLanels",commentEntity)
        return Result1.success200("评论成功")
    }
    ...
    @Encrypt
    @ResponseBody
    fun findComment(@ModelAttribute("data") data: String?, request : HttpServletRequest):BaseData<Any?>{
        logger.info("LanelsController-findLanels",data)
        if (CheckReceivedDataUtil.JsonToClass<CommentEntity>(CommentEntity::class.java,data)==null) return Result1.failure300("格式错误!!!")
        val commentEntity = CheckReceivedDataUtil.JsonToClass<CommentEntity>(CommentEntity::class.java, data)
        if (commentEntity==null)  return Result1.failure300("参数不能为空!")

        if (commentEntity.commentid==0) return Result1.failure300("缺少评论ID!")

        val comment = mainService.commentService.queryById(commentEntity.commentid)
        logger.info("LanelsController-findLanels",comment)
        return Result1.success200(comment,"查询成功")
    }
    ...
    @Encrypt
    @ResponseBody
    fun deleteComment(@ModelAttribute("data") data: String?, request : HttpServletRequest):BaseData<Any?>{
    ...
    @Encrypt
    @ResponseBody
    fun upComment(@ModelAttribute("data") data: String?, request : HttpServletRequest):BaseData<Any?>{
        ...
        if (comment==null)  return Result1.failure300("参数不能为空!")

        if (comment.commentid==0) return Result1.failure300("缺少参数评论ID!")
        if (comment.commentContent.isNullOrBlank()) return Result1.failure300("评论内容不能为空!")
        if (comment.userid==0) return Result1.failure300("缺少参数用户ID!")

        val comments = mainService.commentService.queryById(comment.commentid)
        //先查 后删除
        if (comments.commentid==0) return Result1.failure300("修改失败，该评论不存在!!")
    ...
    @Encrypt
    @ResponseBody
    fun upCommentCount(@ModelAttribute("data") data: String?, request : HttpServletRequest):BaseData<Any?>{
        logger.info("LanelsController-UpLanels",data)
        if (CheckReceivedDataUtil.JsonToClass<CommentEntity>(CommentEntity::class.java,data)==null) return Result1.failure300("格式错误!!!")
        val comment = CheckReceivedDataUtil.JsonToClass<CommentEntity>(CommentEntity::class.java, data)
        if (comment==null)  return Result1.failure300("参数不能为空!")
